...
class SolveTSPUsingACO:
    ...
    def plot2(self, points, path: list):
        x = []
        y = []
        for point in points:
            x.append(point[0])
...
if __name__ == '__main__':
    ...
    acs = SolveTSPUsingACO(mode='ACS', colony_size=_colony_size, steps=_steps, nodes=points)
    acs.run()
    acs.plot()
    elitist = SolveTSPUsingACO(mode='Elitist', colony_size=_colony_size, steps=_steps, nodes=points)
    elitist.run()
    elitist.plot()
    max_min = SolveTSPUsingACO(mode='MaxMin', colony_size=_colony_size, steps=_steps, nodes=points)
    max_min.run()
    max_min.plot()
